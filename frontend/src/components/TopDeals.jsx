...
export default function TopDeals({ user }) {
  ...
  return (
    <section className="bg-white rounded-3xl shadow-xl border border-gray-100 overflow-hidden my-4 sm:my-8 relative">
      ...
      <AnimatePresence>
        {notification && (
          <motion.div
            ...
            className="fixed bottom-4 left-4 right-4 sm:bottom-10 sm:left-5 sm:right-auto z-[100] bg-white/90 backdrop-blur-xl border-2 border-green-300 shadow-[0_10px_40px_rgba(74,222,128,0.3)] p-3 sm:p-4 rounded-2xl flex items-center gap-3 sm:gap-4 sm:min-w-[300px]"
          >
            <div className="bg-green-100 p-2 rounded-full relative shrink-0">
      ...
      <div className="bg-white border-b border-gray-100 py-3 px-4 sm:px-6 flex items-center overflow-hidden relative">
        
        {/* Compteur Fixe à gauche */}
        <div className="flex items-center gap-2 shrink-0 border-r border-gray-200 pr-3 sm:pr-4 mr-3 sm:mr-4 bg-white z-10">
        ...
        <div className="flex-1 overflow-hidden relative">
                 ...
                 {connectedUsers.map((u) => (
                   <div key={`${groupIndex}-${u.id}`} className="flex items-center gap-2 group cursor-pointer bg-gray-50 py-1.5 px-3 rounded-full shadow-sm border border-gray-200 hover:border-green-400 hover:bg-green-50 transition-all">
                     ...
                     <div className="relative">
                       <img src={u.img} alt={u.name} className="w-6 h-6 sm:w-7 sm:h-7 rounded-full object-cover border border-gray-200" />
           ...
           <div className="absolute inset-y-0 left-0 w-6 sm:w-8 bg-gradient-to-r from-white to-transparent z-10 pointer-events-none"></div>
           <div className="absolute inset-y-0 right-0 w-6 sm:w-8 bg-gradient-to-l from-white to-transparent z-10 pointer-events-none"></div>
        </div>
      </div>
      ...
      <div className="p-3 sm:p-5 bg-gray-50/20">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3 sm:gap-6">
          {topDeals.map((deal) => (
            <div
              key={deal.id}
              className="group bg-white rounded-2xl border border-gray-100 hover:border-orange-200 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col overflow-hidden relative"
            >
              <div className="absolute top-2 left-2 sm:top-3 sm:left-3 z-10 bg-red-600 text-white text-[10px] sm:text-xs font-black px-2 py-0.5 sm:py-1 rounded-full shadow-lg transform -rotate-3 group-hover:rotate-0 transition-transform">
                {deal.discount}
              </div>
              
              <Link to={`/product/${deal.id}`} className="block relative aspect-square bg-white p-3 sm:p-6 overflow-hidden flex items-center justify-center">
                <img src={deal.img} alt={`Acheter ${deal.name}`} className="max-h-full max-w-full object-contain transition-transform duration-700 group-hover:scale-110" loading="lazy" />
              ...
              <div className="p-3 sm:p-4 flex flex-col flex-1 bg-white">
                <Link to={`/product/${deal.id}`}>
                  <h3 className="text-[12px] sm:text-[13px] font-bold text-gray-800 line-clamp-1 group-hover:text-[#e96711] transition-colors mb-1">{deal.name}</h3>
                </Link>
                <p className="hidden sm:block text-[11px] text-gray-500 line-clamp-2 leading-relaxed min-h-[32px]">{deal.description}</p>
                
                <div className="h-px w-8 bg-orange-200 my-2 sm:my-3 group-hover:w-full transition-all duration-500"></div>
                
                <div className="flex flex-col sm:flex-row sm:items-center gap-0.5 sm:gap-2 mb-2 sm:mb-3">
                  <span className="text-sm sm:text-lg font-black text-gray-900 leading-none">{deal.price}</span>
                  <span className="text-[10px] sm:text-xs line-through text-gray-400 font-medium decoration-red-400">{deal.oldPrice}</span>
                </div>
                
                <div className="bg-green-50 border border-green-100 rounded-lg sm:rounded-xl p-1.5 sm:p-2.5 flex items-center justify-between group/comm mb-3 sm:mb-4 hover:bg-green-100/50 transition-colors">
                  <div className="flex items-center gap-1 sm:gap-2">
                    <TrendingUp size={12} className="text-green-500 sm:w-3.5 sm:h-3.5" strokeWidth={3} />
                    <span className="text-[9px] sm:text-[10px] font-bold text-green-800 uppercase hidden sm:inline">Commission</span>
                    <span className="text-[9px] font-bold text-green-800 uppercase sm:hidden">Com.</span>
                  </div>
                  <span className="text-[10px] sm:text-xs font-black text-green-600 group-hover/comm:scale-110 transition-transform">{deal.commission}</span>
                </div>
                
                <button
                  onClick={(e) => handleShare(e, deal)}
                  className="mt-auto group/btn relative overflow-hidden w-full bg-blue-600 hover:bg-blue-700 text-white py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl font-bold text-[10px] sm:text-xs flex items-center justify-center gap-1.5 sm:gap-2.5 shadow-md hover:shadow-lg active:scale-95 transition-all"
                >
                  ...
                  <Share2 size={14} strokeWidth={2.5} className="relative z-10 sm:w-4 sm:h-4" />
                  <span className="relative z-10 whitespace-nowrap">Partager <span className="hidden sm:inline">le lien</span></span>
                </button>
              </div>
            </div>
          ))}
