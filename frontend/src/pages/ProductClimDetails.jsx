...
import { climatiseurs } from "../components/ClimatisationVentioateurs"; // 🔹 Assure-toi d'exporter climatiseurs
import { FaFacebook, FaInstagram, FaWhatsapp } from "react-icons/fa";

export default function ProductClimDetails({ user }) {
  const { id } = useParams();
  const product = climatiseurs.find((p) => p.id === Number(id));
  ...
  // 🔹 Avis fictifs
  const reviews = [
    { name: "Ali", rating: 5, comment: "Très efficace et silencieux 👍", img: "/images/avis/1.jpg" },
    { name: "Koffi", rating: 4, comment: "Refroidit rapidement", img: "/images/avis/2.jpg" },
    { name: "Marie", rating: 5, comment: "Super produit, livraison rapide", img: "/images/avis/3.jpg" },
  ];
    ...
    <div className="bg-gray-100 min-h-screen py-6">
      <div className="max-w-7xl mx-auto grid md:grid-cols-3 gap-6 px-4">
        ...
        <div className="bg-white p-6 rounded shadow">
          ...
          <div className="text-3xl font-bold">{product.price.toLocaleString()} FCFA</div>
          {product.oldPrice && (
            <div className="ml-3 line-through text-gray-400">{product.oldPrice.toLocaleString()} FCFA</div>
          )}
          {product.discount && (
            <div className="ml-3 bg-orange-100 text-orange-600 px-2 py-1 rounded">{product.discount}</div>
          )}

          <p className="mt-4 text-gray-600">{product.description || "Description non disponible."}</p>

          <p className="text-green-600 mt-3">Disponible</p>
          <p className="text-sm text-gray-600 mt-2">
            + livraison à partir de <span className="font-semibold ml-1">1000 FCFA</span>
          </p>
        ...
        <div className="bg-white p-6 rounded shadow">
          ...
          <div className="border p-4 rounded">
            ...
            <p className="mt-2 font-semibold">ClimStore Official</p>
